            ...
            db.pool.getConnection(function(err, connection) {
                ...
                LIMIT 3`, function(error, result,field) {
                    if (error) throw error;
                        ...
                        .addFields(
                            {name: `${medals[0]}`, value: `${capitalizeFirstLetter(result[0]['nomRp'].replace('-',' '))} : ${result[0]['totalKg']} kg`},
                            {name: `${medals[1]}`, value: `${capitalizeFirstLetter(result[1]['nomRp'].replace('-',' '))} : ${result[1]['totalKg']} kg`},
                            {name: `${medals[2]}`, value: `${capitalizeFirstLetter(result[2]['nomRp'].replace('-',' '))} : ${result[2]['totalKg']} kg`}
                        )
                        .setColor('#E67E22')
                        .setFooter('© Ferme')
                        .setTimestamp();
